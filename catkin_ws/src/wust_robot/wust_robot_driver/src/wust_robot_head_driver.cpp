...
namespace ros_control_mr {
  mrDriver::mrDriver(ros::NodeHandle nh) :
          ...
          as_(nh_, "head_controller/follow_joint_trajectory",
          ...
          16, 0.0)
  {
    ...
    if (ros::param::get("/head_hardware_interface/peak_can_device/num", buf_int)) {
      ROS_INFO("Number of Peak CAN devices: %d", buf_int);
    } else {
      ROS_ERROR("Failed to get param '/head_hardware_interface/peak_can_device/num'");
    }
    num_devices_ = buf_int;
    // start masters(Peak CAN devices)
    for (std::size_t i = 0; i < num_devices_; ++i) {
      std::sprintf(device_key, "/head_hardware_interface/peak_can_device/device%d", (int)i);
    ...
    ros::param::get("/head_hardware_interface/joints", joint_names_);
    ...
    nh_.getParam("/head_hardware_interface/jointID", jointID_);
    ...
    // grippers
//    ros::param::get("/hardware_interface/grippers", gripper_names_);
...
//      exit(-1);
//    }
//    nh_.getParam("/hardware_interface/gripperID", gripperID_);
...
//    hGripper_.resize(num_grippers_);
    ...
    if (ros::param::get("/head_hardware_control_loop/loop_hz", loop_hz)) {
    ...
    for (std::size_t i = 0; i < num_joints_; ++i) {
      ...
      ros::param::get("/head_hardware_interface/jointDevice", map);
...
//      int masterId;
//      std::vector<std::string> map;
//      ros::param::get("/hardware_interface/gripperDevice", map);
//      std::sscanf(map.at(i).c_str(), "device%d", &masterId);
...
//    }
    ...
    position_sub_ = nh_.subscribe("joint_cmd", 1, &mrDriver::jointInterface, this);
//    gripper_sub_ = nh_.subscribe("gripper_cmd", 1, &mrDriver::gripperInterface, this);
  }
...
//  }
  ...
  void mrDriver::publishRTMsg() {
    float pos, vel, curr;
    ros::Publisher joint_pub = nh_.advertise<sensor_msgs::JointState>("/head/joint_states", 1);
//    ros::Publisher gripper_pub = nh_.advertise<mr_msgs::GripperState>("/gripper_states", 1);
