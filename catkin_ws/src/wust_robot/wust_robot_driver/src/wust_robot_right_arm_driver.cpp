...
namespace ros_control_mr {
  mrDriver::mrDriver(ros::NodeHandle nh) :
          ...
          as_(nh_, "right_arm_controller/follow_joint_trajectory",
          ...
          16, 0.0)
  {
    ...
    if (ros::param::get("/right_arm_hardware_interface/peak_can_device/num", buf_int)) {
      ROS_INFO("Number of Peak CAN devices: %d", buf_int);
    } else {
      ROS_ERROR("Failed to get param '/right_arm_hardware_interface/peak_can_device/num'");
    }
    num_devices_ = buf_int;
    // start masters(Peak CAN devices)
    for (std::size_t i = 0; i < num_devices_; ++i) {
      std::sprintf(device_key, "/right_arm_hardware_interface/peak_can_device/device%d", (int)i);
    ...
    ros::param::get("/right_arm_hardware_interface/joints", joint_names_);
    ...
    nh_.getParam("/right_arm_hardware_interface/jointID", jointID_);
    ...
    // grippers
    ros::param::get("/right_arm_hardware_interface/grippers", gripper_names_);
    ...
    nh_.getParam("/right_arm_hardware_interface/gripperID", gripperID_);
    ...
    if (ros::param::get("/right_arm_hardware_control_loop/loop_hz", loop_hz)) {
    ...
    for (std::size_t i = 0; i < num_joints_; ++i) {
      ...
      ros::param::get("/right_arm_hardware_interface/jointDevice", map);
    ...
    for (std::size_t i = 0; i < num_grippers_; ++i) {
      int masterId;
      std::vector<std::string> map;
      ros::param::get("/right_arm_hardware_interface/gripperDevice", map);
      std::sscanf(map.at(i).c_str(), "device%d", &masterId);
...
//    }
    ...
    position_sub_ = nh_.subscribe("right_arm/joint_cmd", 1, &mrDriver::jointInterface, this);
    gripper_sub_ = nh_.subscribe("right_gripper/gripper_cmd", 1, &mrDriver::gripperInterface, this);
  }
...
//      }
  ...
  void mrDriver::publishRTMsg() {
    float pos, vel, curr;
    ros::Publisher joint_pub = nh_.advertise<sensor_msgs::JointState>("/right_arm/joint_states", 1);
//    ros::Publisher gripper_pub = nh_.advertise<mr_msgs::GripperState>("/right_gripper_states", 1);
    ros::Publisher gripper_joint_pub = nh_.advertise<sensor_msgs::JointState>("/right_gripper/joint_states", 1);
...
//      gripper_pub.publish(gripper_msg);
      ...
      gripper_joint_msg.name = {"R_left_joint", "R_right_joint"};
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "right_arm_driver");
  ros::NodeHandle nh;
